...
class MessageSerializer(serializers.Serializer):
    ...
    def __init__(self, queryset=None, *args, **kwargs):
        ...
        if (
            ...
            and not queryset[0].is_read
            and request.user != queryset[0].sender
        ):
            message = queryset[0]
            chat = Chat.objects.get(messages=message)
            message_id = [str(i.id) for i in queryset]

            async_to_sync(read_receipt_notification)(
                message.sender, message_id, chat.id
            )
