...
class MessageModel(TimeStampedUUIDModel):
    sender = models.ForeignKey(
        User, related_name="sender_messages", on_delete=models.SET_NULL, null=True
    )
    text = models.TextField(null=True, blank=True)
    image = models.ImageField(
        verbose_name=(_("Image")),
        upload_to="static/chat_images/",
        null=True,
        blank=True,
    )
...
class Chat(TimeStampedUUIDModel):
    user1 = models.ForeignKey(
        User, related_name="user1_chat", on_delete=models.SET_NULL, null=True
    )
    user2 = models.ForeignKey(
        User, related_name="user2_chat", on_delete=models.SET_NULL, null=True
    )
